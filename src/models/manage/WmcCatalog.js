import { getCatalogPage,saveOrUpdateCatalog,deleteCatalog } from '../../services/WmcManageSvc'
export default {
  namespace: 'WmcCatalog',
  state: {
    submit: false,
    list: {
      data: [],
  ...
  effects: {
    ...
    *saveOrUpdate({ payload }, { call, put }) {
      ...
      let { data } = yield saveOrUpdateCatalog( payload.values);
      if (data) {
        if (data.errorCode == "suc") {
          payload.back()
        } else if (data.errorCode == "auth") {
