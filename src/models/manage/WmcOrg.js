import { getOrgPage,saveOrUpdateOrg,deleteOrg } from '../../services/WmcManageSvc'
export default {
  namespace: 'WmcOrg',
  state: {
    submit: false,
  ...
  effects: {
    ...
    *saveOrUpdate({ payload }, { call, put }) {
      ...
      let { data } = yield saveOrUpdateOrg( payload.values);
      if (data) {
        if (data.errorCode == "suc") {
          payload.back()
        } else if (data.errorCode == "auth") {
