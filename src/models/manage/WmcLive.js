import { getLivePage,saveOrUpdateLive,deleteLive } from '../../services/WmcManageSvc'
export default {
  namespace: 'WmcLive',
  state: {
    submit: false,
  ...
  effects: {
    ...
    *saveOrUpdate({ payload }, { call, put }) {
      ...
      let { data } = yield saveOrUpdateLive( payload.values);
      if (data) {
        if (data.errorCode == "suc") {
          payload.back()
        } else if (data.errorCode == "auth") {
