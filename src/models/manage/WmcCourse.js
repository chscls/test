import { getCoursePage,saveOrUpdateCourse,deleteCourse } from '../../services/WmcManageSvc'
export default {
  namespace: 'WmcCourse',
  state: {
    submit: false,
  ...
  effects: {
    ...
    *saveOrUpdate({ payload }, { call, put }) {
      ...
      let { data } = yield saveOrUpdateCourse( payload.values);
      if (data) {
        if (data.errorCode == "suc") {
          payload.back()
        } else if (data.errorCode == "auth") {
