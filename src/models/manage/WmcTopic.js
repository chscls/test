import { getTopicPage,saveOrUpdateTopic,deleteTopic } from '../../services/WmcManageSvc'
export default {
  namespace: 'WmcTopic',
  state: {
    submit: false,
  ...
  effects: {
    ...
    *saveOrUpdate({ payload }, { call, put }) {
      ...
      let { data } = yield saveOrUpdateTopic( payload.values);
      if (data) {
        if (data.errorCode == "suc") {
          payload.back()
        } else if (data.errorCode == "auth") {
